...
- `ViewData` is allowed for `Title` and the existing public-theme SEO values (`MetaTitle`, `MetaDescription`, `OgImage`, `OgType`, `CanonicalUrl`).
...
- Use Bootstrap and existing CSS conventions. Public theme overrides live under `Areas/Blog/Themes/` and `styles/themes/`.
...
- Edit editor source in `ClientApp/` and theme source in `styles/themes/`; do not manually edit their generated `wwwroot` outputs.
- Keep markup accessible: labels for inputs, meaningful button text or labels, image alt text, and semantic elements.
