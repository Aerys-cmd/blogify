...
- PageModels are request orchestrators: query, call domain behavior, save, return. Focused services are valid for shared permissions, storage, feeds, analytics, rendering, and other infrastructure behavior.
...
- Use `TenantContext.RequiredTenant` or `CurrentTenantId`; never authorize or scope data from a submitted tenant ID alone.
- Global tenant/soft-delete filters exist for `Post`, `Category`, `Media`, and `Comment`. Explicitly scope entities without those filters, especially `Tag`, `BlogMembership`, `BlogInvitation`, and `AnalyticsEvent`.
